...
fn start(command: &str) -> Result<(), Box<dyn Error>> {
    let mut child = Command::new("bash").args(["-c", &command]).spawn()?;
...
fn cache_wallpaper(
    ...
    cached_wallpapers_path: &PathBuf,
) {
    ...
    for (i, cached_wallpaper_name) in cached_wallpaper_names.iter().enumerate() {
        let path = format!(
            "{}/{}",
            cached_wallpapers_path.display(),
            cached_wallpaper_name
        );
        ...
        if !Path::new(&path).exists() {
            ...
            let thread = thread::spawn(move || {
                ...
                let _ = img.save(config::parse_path(&format!(
...
fn apply_templates(
    ...
    variables: Vec<colorvariables::ColorVariable>,
    wal_color_path: PathBuf,
) {
...
fn parse_args(default_config_path: PathBuf) -> (PathBuf, String, bool) {
    ...
    for (i, param) in args.iter().enumerate() {
        match param.as_str() {
            "-c" | "--conf" => config_path = config::parse_path(&args[i + 1]),
